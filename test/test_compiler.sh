...
test_stage () {
    ...
    echo "STAGE $1"
    echo "===================Valid Programs==================="
    for prog in `find . -type f -name "*.c" -path "./test/stage_$1/valid/*" -not -path "*/valid_multifile/*" 2>/dev/null`; do
    ...
    for dir in `ls -d ./test/stage_$1/valid_multifile/* 2>/dev/null` ; do
        gcc -w $dir/*
    ...
    for prog in `ls ./test/stage_$1/invalid/{,**/}*.c 2>/dev/null`; do
